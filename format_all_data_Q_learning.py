"""
This script formats the data desired into OF and NF 3D matrices including Q-step information used as input to the models
"""
...
Q = 3  # The number of stations steps ahead to make the prediction
...
all_values_actual_interval_between_trains = [value for sublist in
                                             station_averages_actual_interval_between_trains.values()
...
all_station_average_planned_interval_between_trains = float(math.ceil(
    sum(station_average_planned_interval_between_trains.values()) / len(
        station_average_planned_interval_between_trains)))
...
for k, date in enumerate(sorted_trip_id_to_departure_time):
    ...
    for train in sorted_trip_id_to_departure_time[date]:
        ...
        for train_station in train_line:
                ...
                for j in range(P - Z + 1, P + 1):
                    ...
                    if current_P - Z >= 0 and current_I - H >= 0:
                        ...
                        if prev_train_id is not None or train_id is not None:
                            if (date in actual_interval_between_trains and station in
                                    actual_interval_between_trains[date]):
            ...
            a = 0
            b = 0
            # Fill OF matrix with planned running time features including Q-step information
            for i in range(I - H + 1, I + 1):
                ...
                for j in range(P - Z + 1, P + 1):
                    ...
                    if current_P - Z >= 0 and current_I - H >= 0:
                        station = train_line[current_P - Z + b - 1]
                        # If we are on the current station which is the last column of each
                        # row then we should fill it with Q-Step information
                        if b == P:
                            Q_station_list = []
                            Q_station_list.append(station)
                            # We want to iterate from current station to station Q
                            for i in range(0, Q):
                                # Extract station names of the Q stations
                                if 0 <= current_P - Z + b + i < len(train_line):
                                    Q_next_station = train_line[current_P - Z + b + i]
                                    Q_station_list.append(Q_next_station)
                                else:
                                    next_station = None
                                    Q_station_list.append(next_station)
                            train_id = list(sorted_trip_id_to_departure_time[date].keys())[current_I - H + a - 1]
                            Q_running_times = []
                            # Extract the planned running times in the interval from current station to
                            # station Q of a specific train
                            for i in range(len(Q_station_list) - 1):
                                current_station = Q_station_list[i]
                                next_station = Q_station_list[i + 1]
                                if next_station is not None:
                                    if train_id in all_planned_running_times[k][date2]:
                                        running_time_stations = current_station + "-" + next_station
                                        ...
                                        tprime = station_averages_planned_running_times[running_time_stations]
                                        Q_running_times.append(tprime)
                            if len(Q_running_times) == 0:
                                OF[a - 1][b - 1 + (Z * 5)] = 0
                            else:
                                OF[a - 1][b - 1 + (Z * 5)] = sum(Q_running_times)

                        else:  # Follows the Q=1 approach which does not include Q-step
            ...
            a = 0
            b = 0
            # Fill OF matrix with planned dwell time features including Q-step prediction
            for i in range(I - H + 1, I + 1):
                a += 1
                for j in range(P - Z + 1, P + 1):
                    b += 1
                    if current_P - Z >= 0 and current_I - H >= 0:
                        ...
                        if b == P:
                            Q_station_list = []
                            # Extract the planned dwell times in the interval from current station to
                            # station Q of a specific train
                            for i in range(0, Q - 1):
                                if 0 <= current_P - Z + b + i < len(train_line):
                                    Q_station = train_line[current_P - Z + b + i - 1]
                                    Q_station_list.append(Q_station)
                                else:
                                    Q_station = None
                                    Q_station_list.append(Q_station)
                            train_id = list(sorted_trip_id_to_departure_time[date].keys())[current_I - H + a - 1]
                            Q_dwell_times = []
                            for i in range(len(Q_station_list)):
                                current_station = Q_station_list[i]
                                if current_station is not None:
                                    planned_dwell_time = 0
                                    if current_station in dwell_times_after_allowable[train_id]:
                                        planned_dwell_time = dwell_times_after_allowable[train_id][current_station]
                                    else:
                                        planned_dwell_time = station_averages_planned_dwell_times[current_station]
                                    wprime = planned_dwell_time
                                    wprime = float(round(wprime))
                                    Q_dwell_times.append(wprime)
                            if len(Q_dwell_times) == 0:
                                OF[a - 1][b - 1 + (Z * 6)] = 0
                            else:
                                # Sum all of the dwell times from current station to last Q station
                                OF[a - 1][b - 1 + (Z * 6)] = sum(Q_dwell_times)
            ...
            a = 0
            b = 0
            # Fill OF matrix with planned interval time between train i and i+1 features including Q-step prediction
            for i in range(I - H + 1, I + 1):
                a += 1
                for j in range(P - Z + 1, P + 1):
                    b += 1
                    ...
                    if current_P - Z >= 0 and current_I - H >= 0:
                        ...
                        if b == P:
                            ...
                            for i in range(len(Q_station_list)):
                                ...
                                if current_station is not None:
                                    suppliment_time = 0
                                    # The first station of line 13 and line 14 do not have a supplement running time
                                    if current_station != "Norsborg" or station != "Fruängen":
                                        if train_id in all_suppliment_buffer[k][date2]:
                                            if current_station in all_suppliment_buffer[k][date2][train_id]:
                                                suppliment_time = all_suppliment_buffer[k][date2][train_id][current_station]
                                            else:
                                                suppliment_time = station_averages_suppliment_buffer[current_station]
                                            Q_suppliment_times.append(suppliment_time)
                                        else:
                                            suppliment_time = station_averages_suppliment_buffer[current_station]
                            if len(Q_suppliment_times) == 0:
                                OF[a - 1][b - 1 + (Z * 8)] = 0
                            else:
                                # Sum all of the dwell times from current station to last Q station
                                OF[a - 1][b - 1 + (Z * 8)] = sum(Q_suppliment_times)

                        # Determine prev_station based on j value
                        else:
                            # Determine train_id and running_time_stations based on i and j values
                            train_id = list(sorted_trip_id_to_departure_time[date].keys())[current_I - H + a - 1]
                            if station == "Norsborg" or station == "Fruängen":
                                suppliment_time = 0
                            else:
                                if train_id in all_suppliment_buffer[k][date2]:
                                    ...
                                    sprime = station_averages_suppliment_buffer[station]
